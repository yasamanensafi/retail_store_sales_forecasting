...
At first, classical time- series forecasting methods like Seasonal ARIMA and Triple Exponential Smoothing have been used and then, more current methods like Prophet, Long Short-Term Memory (LSTM), and Convolutional Neural Network (CNN) have been applied. 
...
##  The  Dataset
The dataset used for this project describes Superstore Sales from 2014 to the end of 2017 and it contains nearly 10,000 rows and 21 features. The dataset consists of sales information of three different categories, furniture, technology, and office supplies. In this project, the sales of furniture is the variable of interest because it contains seasonal patterns.
...
## Train-Test Split
For the task of  time-series forecasting we cannot divide the dataset to train set and test set randomly. In fact, observations in time-series are dependent on each other and data should be split in time. Therefore, common techniques such as the k-fold cross validation doesn’t work for time-series. In this project, 75% of data has been used for training set which is equal to the first three years of sales history and the rest of the data which is equal to last year of sales history has been used for testing the model. 
## Notebooks
